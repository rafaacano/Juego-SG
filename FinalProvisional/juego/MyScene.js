 ...
 */
 class MyScene extends THREE.Scene {
    ...
    update () {
      ...
      if(this.model.getPosX() > 167){
        ...
        if(reajusteRueda == true){
          this.model.position.set(168, 1.5, 0);
          reajusteRueda = false;
        }
      }
      /************************************************************/
    ...
    colisionCubos(){
      ...
      /*
      if( movNave ){
        var rayo2 = new THREE.Raycaster(this.model.position, new THREE.Vector3(0,-1,0),0,0.5);
        var rayo3 = new THREE.Raycaster(this.model.position, new THREE.Vector3(0,1,0),0,0.5);
      }
      */

      for(let i=0; i < cubos.length; i++){
        var interseccion = rayo.intersectObject(cubos[i]);
        //var interseccion2 = rayo2.intersectObject(cubos[i]);
        //var interseccion3 = rayo3.intersectObject(cubos[i]);
    ...
    cambio(x,y,z){
      ...
      this.camino1 = new THREE.CatmullRomCurve3(
        [ new THREE.Vector3(x,y,z),
            ...
            new THREE.Vector3(x+9.5,y+18.5,z)]
      );
